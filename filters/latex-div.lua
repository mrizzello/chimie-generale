--[[
     ...
     Copied from the rmarkdown R package (rmarkdown/lua/latex-div.lua) for use
     under Quarto, which doesn't set the RMARKDOWN_LUA_SHARED environment
     variable that the original file's version-guard preamble depends on.
     That preamble only guarded against Pandoc < 2.1, which is irrelevant for
     any Pandoc Quarto bundles, so it has been removed here; the filter body
