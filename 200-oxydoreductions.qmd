...
```{r fig-gamma-rule,echo=FALSE,out.width="45%",fig.show="hold",fig.align="center",fig.cap="Illustration de la règle du gamma."}
...
```{r, child=c('exe/oxydoreductions/35.Rmd')}
```
...
```{r, child=c('exe/oxydoreductions/160.Rmd')}
```

```{r, child=c('exe/oxydoreductions/170.Rmd')}
```
