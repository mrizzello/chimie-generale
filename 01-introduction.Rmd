...
$$\textrm{eau à l'état liquide }\ \xrightarrow{\textrm{évaporation}}\ \textrm{eau à l'état gazeux}$$
...
$$\textrm{papier}\ +\ \textrm{oxygène} \ \xrightarrow{\textrm{combustion}} \ \textrm{dioxyde de carbone} + \textrm{eau}$$

Les transformations chimiques sont ce que nous appellerons plus tard des **réactions chimiques**.

```{r, child=c('_exe/1-introduction-10.Rmd')}
```
...
```{r, child=c('_exe/1-introduction-20.Rmd')}
```

## Multiples des unités

Multiples et sous-multiples décimaux des unités du système international (SI) :
...
```{r, child=c('_exe/1-introduction-30.Rmd')}
```

## Exercices supplémentaires

```{r, child=c('_exe/1-introduction-40.Rmd')}
```

```{r, child=c('_exe/1-introduction-50.Rmd')}
```

```{r, child=c('_exe/1-introduction-60.Rmd')}
```

```{r, echo=FALSE}
if (knitr::is_latex_output()) {
  knitr::asis_output('\\section{Solutions des exercices}')
  knitr::asis_output('\\shipoutAnswer')
}
```
