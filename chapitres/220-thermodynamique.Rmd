...
Sur la base de ces exemples et de ceux que nous avons vu dans le chapiitre sur le thermochimie, nous pourrions conclure que les réactions exothermiques ont tendance à être spontanées. Cependant, lorsque la glace fond, le processus est endothermique et spontané.
...
| Le sodium métallique réagit violemment avec l'eau pour produire de l'hydroxyde de sodium et du gaz d'hydrogène. | L'hydroxyde de sodium réagit avec l'hydrogène pour produire du sodium métallique et de l'eau. |
...
**Entropie - S**  
Mesure du degré de désordre d'un système.  
(Exprimée en joules par Kelvin [J/K]).

:::

**Plus le désordre est grand, plus l'entropie est élevée.**

Les solides sont plus ordonnés que les liquides et les liquides sont plus désordonnés les gaz. On peut classer les états de la matière par ordre d'entropie :
...
```{r cards-castle,echo=FALSE,out.width="67%",fig.show="hold",fig.align="center",fig.cap="Deuxième loi de la thermodynamique"}
...
### Entropie molaire standard ($S^o$) et entropie de réaction ($\Delta S^o$)
...
$$
\Delta S^o = \sum n_p \cdot S^{o}_{(produits)} - \sum n_r \cdot S^{o}_{(reactifs)}
$$

$n_p$ et $n_r$ sont les coéfficients stoechiométriques des réactifs et produits de la réaction dont on effectue le calcul.

## Énergie libre ($G$) et variation d'énergie libre ($\Delta G$)
...
La **variation de l'énergie libre** ($\Delta G$), d'un système pour une réaction :
...
Table: (\#tab:spontaneiteDG) Spontanéité d'une réaction selon le signe de $\Delta G$.

|                |                                              |                           |
| :------------: | :------------------------------------------- | :------------------------ |
| $\Delta G < 0$ | Réaction est spontanée dans le sens direct   | **Réaction exergonique**  |
| $\Delta G > 0$ | Réaction est spontanée dans le sens indirect | **Réaction endergonique** |
| $\Delta G = 0$ | Le système est à l'équilibre                 |                           |

## Énergie libre standard de réaction ($\Delta G^o$)

Tout comme pour l'enthalpie et l'entropie, afin de travailler avec des valeur de référence, on introduit l'**énergie libre standard de réaction**. L'énergie libre standard de réaction ($\Delta G^o$) est la variation d'énergie libre pour une réaction lorsqu'elle se produit dans des conditions d'état standard, c'est-à-dire lorsque des réactifs dans leur état standard sont convertis en produits dans leur état standard. La relation précédente devient : 

$$
\Delta G^o = \Delta H^o - T \cdot \Delta S^o
$$

## Énergie libre et équilibre chimique
...
$$
\begin{split}
0 &= \Delta G^o + RT \cdot ln(K) \\
\Rightarrow \Delta G^o &= - RT \cdot ln(K)
\end{split}
$$

```{r dG-profile,echo=FALSE,out.width="75%",fig.show="hold",fig.align="center",fig.cap="Énergie libre standard et transformation spontanée"}
