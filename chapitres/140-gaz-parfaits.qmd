...
```{r fig-theorie-cinetique,echo=FALSE,out.width="20%",fig.show="hold",fig.align="center",fig.cap="Illustration du modèle de la théorie cinétique des gaz"}
...
```{r, child=c('exe/gaz-parfaits/05.Rmd')}
```
...
```{r, child=c('exe/gaz-parfaits/25.Rmd')}
```
...
```{r, child=c('exe/gaz-parfaits/90.Rmd')}
```

\newpage

```{r, child=c('exe/gaz-parfaits/100.Rmd')}
```

```{r, child=c('exe/gaz-parfaits/110.Rmd')}
```
