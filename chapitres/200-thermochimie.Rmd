...
Un objet peut posséder de l'énergie sous deux formes :
...
- Un système est **ouvert** s'il permet des échanges d'énergie et de matière avec le milieu extérieur.
- Un système est **fermé** s'il peut échanger de l'énergie mais pas de matière.
...
\begin{split}
  ...
  \text{Énergie} + \text{Réactifs} & \rightarrow \text{Produits}
\end{split}
\quad
\begin{split}
...
| ![](images/enthalpie-1-1.png){width=30%} | ![](images/enthalpie-1-2.png){width=30%} |
...
m & \text{: masse}\ [J/(K \cdot g)] \\
C & \text{: capacité thermique}\ [J/(K \cdot g)] \\
\Delta T & \text{: différence de température}\ [K]
\end{split}
$$

Un calorimètre mesure le changement de température accompagnant un processus. Le changement de température d'un calorimètre dépend de sa capacité thermique, c'est-à-dire de la quantité de chaleur nécessaire pour élever sa température de 1 K.

Si une expérience de calorimétrie est réalisée sous une pression constante, la chaleur transférée fournit une mesure directe de la variation d'enthalpie de la réaction. La calorimétrie à volume constant est réalisée dans un récipient de volume fixe appelé calorimètre à bombe. La chaleur transférée dans des conditions de volume constant est égale à ∆E. Des corrections peuvent être appliquées aux valeurs de ∆E pour donner ∆H.
