...
```{r fig-ordre-grandeur-atome,echo=FALSE,out.width="33%",fig.show="hold",fig.align="center",fig.cap="Taille et structure de l'atome"}
...
```{r fig-isotopes-table,echo=FALSE,out.width="80%",fig.show="hold",fig.align="center",fig.cap="Table des isotopes simplifiée"}
...
```{r fig-uma-masse-molaire,echo=FALSE,out.width="67%",fig.show="hold",fig.align="center",fig.cap="Relation entre unité de masse atomique et masse molaire."}
...
```{r, child=c('exe/atome/210.Rmd')}
```

```{r, child=c('exe/atome/220.Rmd')}
```

\newpage

```{r, child=c('exe/atome/230.Rmd')}
```

```{r, child=c('exe/atome/240.Rmd')}
```
